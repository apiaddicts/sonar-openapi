...
public class AsyncApiProfileDefinition implements BuiltInQualityProfilesDefinition {
  ...
  public void define(BuiltInQualityProfilesDefinition.Context context) {
    NewBuiltInQualityProfile profile = context.createBuiltInQualityProfile(SONAR_WAY_PROFILE, AsyncApi.KEY);
    profile.setDefault(true);
    for (Class<?> check : AsyncApiCheckList.getChecks()) {
      Rule annotation = AnnotationUtils.getAnnotation(check, Rule.class);
      profile.activateRule(AsyncApiCheckList.REPOSITORY_KEY, annotation.key());
    }
    profile.done();
  }
}
